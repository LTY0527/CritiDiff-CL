...
def test_revised_manifest_has_complete_windowref_coverage_and_no_outer_access():
    audit = json.loads(Path("configs/protocol_manifests/protocol_amendment_audit.json").read_text(encoding="utf-8"))
    manifest = json.loads(Path("configs/protocol_manifests/dry_run_manifest.json").read_text(encoding="utf-8"))
